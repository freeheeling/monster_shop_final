...
RSpec.describe 'As an Admin User' do
  describe 'when I visit the merchant index page' do
    it 'clicking a merchant name redirects to an admin merchant show page idential to that merchants dashboard' do
      ...
      merchant = Merchant.create!( name: "Brian's Bike Shop", address: '123 Bike Rd.', city: 'Richmond', state: 'VA', zip: 23_137)

      site_admin = User.create(name: 'Site Admin', email: 'site_admin@user.com', password: 'secure', role: 3)
      address= site_admin.addresses.create(address: '123 First', city: 'Denver', state: 'CO', zip: 80_233)
      ...
      order = user.orders.create!(name: 'Meg', address_id: address.id, status: 'Pending')
